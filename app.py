...
def index():
    ...
    portfolio = db.execute(
        "SELECT symbol, shares, price, total FROM portfolio WHERE user_id = ?", user_id)
...
def register():
    ...
    if request.method == "POST":
        ...
        # Hash the password
        # default method scrypt errored in macos
...
def buy():
    ...
    if request.method == "POST":
        ...
        quote = lookup(symbol)
        if not quote:
            return apology("symbol not listed in C$50 finance", 400)
        ...
        # Flash a success message
        flash(
            f"Bought {shares} shares of {quote['name']} ({quote['symbol']}) at ${quote['price']:.2f} each.")
...
def history():
    ...
    rows = db.execute(
        "SELECT symbol, shares, price, time FROM history WHERE user_id = ?", session["user_id"])
...
def sell():
    ...
    if request.method == "POST":
        ...
        row = db.execute(
            "SELECT symbol, shares FROM portfolio WHERE user_id = ? AND symbol = ?", session["user_id"], symbol)
        if len(row) == 0:
        ...
        # Flash a success message
        flash(
            f"Sold {shares} shares of {quote['name']} ({quote['symbol']}) at ${quote['price']:.2f} each.")
